...
const MapMonitor = () => {
  ...
  const notificationIdCounterRef = useRef(0); // Define notificationIdCounterRef
          ...
          if (circle) {
            ...
            if (distance > circle.radius) {
              ...
              setNotifications(prevNotifications => [
                ...prevNotifications,
                { id: notificationIdCounterRef.current++, message: `${title} is out of the sector area for ${sector.title}.` }
              ]);
            }
          } else if (polygon) {
